...
import { Flex, Text, cn, Divider, Badge, TokenIcon } from "@/components/ui";
...
export const SymbolInfoBar: FC<SymbolInfoBarProps> = (props) => {
  ...
  return (
    ...
    >
      <Flex className="oui-flex-1 oui-overflow-hidden">
        <Flex direction="column" itemAlign="start" gap={1} className="oui-min-w-0">
          <Flex gap={1} itemAlign="center" className="oui-min-w-0">
            {symbolView}
            <Badge size="xs" color="primary">
              {leverage}x
            </Badge>
            <TriangleDownIcon className="oui-text-base-contrast-54 oui-w-[14px] oui-h-[14px] oui-text-base-contrast-80" />
            <RwaTooltip
              isRwa={isRwa}
              open={open}
